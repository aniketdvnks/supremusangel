...
BASE = """from `tabSales Invoice` si join `tabSales Team` st on st.parent=si.name
    ...
    where si.docstatus=1 and si.custom_unlisted_shares=1
...
def referral_chain(f):
    ...
    rows = sql("""select st.sales_person,st.custom_commission_tier tier,st.commission_rate percent,st.incentives amount,
        case when st.sales_person=si.custom_primary_agent then 'Direct' else 'Override' end kind
        from `tabSales Team` st join `tabSales Invoice` si on si.name=st.parent and st.parenttype='Sales Invoice'
        where si.name=%(sales_invoice)s and si.docstatus=1 and si.custom_unlisted_shares=1 order by st.idx""", f)
...
def my_sales(f):
    rows = sql("""select si.name sales_invoice,si.posting_date,si.customer,
        ...
        case when si.custom_primary_agent=st.sales_person then 'Direct' else 'Override' end kind,
...
def my_downline(f):
    ...
    rows = sql("""select si.custom_primary_agent sales_person, seller.custom_tier tier,
        ...
        where si.docstatus=1 and si.custom_unlisted_shares=1 and st.sales_person=manager.name
